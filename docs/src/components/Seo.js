...
const ogImage = require('images/svelte-query-og.png?url')
  ...
  ({ title, description, image = ogImage, router, children }) => (
    <Head>
      ...
      <meta
        property="og:url"
        key="og:url"
        content={`https://svelte-query.tanstack.com${router.pathname}`}
      />
      {title != undefined && (
      ...
      {image != undefined && (
        <meta
          property="og:image"
          key="og:image"
          content={`https://svelte-query.tanstack.com${image}`}
        />
      )}
